...
public class TargetWeightCalculator
{
    ...
    {
        if (!settings.Targeting.Weights.EnableWeighting) return 0f;
        ...
        var distanceWeight = (float)Math.Pow(1 - distance / settings.Targeting.MaxTargetDistance, 2) *
                             settings.Targeting.Weights.DistanceWeight;
        ...
        rarityWeight *= (float)Math.Pow(1 - distance / settings.Targeting.MaxTargetDistance, 0.5);
        weight += rarityWeight;
        ...
        if (life != null)
        {
            ...
            weight += hpWeight * (float)Math.Pow(1 - distance / settings.Targeting.MaxTargetDistance, 0.5);
        }
        ...
        {
            var customWeight = settings.Targeting.CustomTargets.Weight;
            // Custom weight also scales with distance
            weight += customWeight * (float)Math.Pow(1 - distance / settings.Targeting.MaxTargetDistance, 0.5);
        }
        ...
        if (distance < settings.Targeting.MaxTargetDistance * 0.2f)
            weight *= 1.5f + (1 - distance / (settings.Targeting.MaxTargetDistance * 0.2f));
